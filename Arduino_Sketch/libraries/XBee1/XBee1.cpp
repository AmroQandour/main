 ...
 *
 * This file is part of XBee1-Arduino.
 *
 * XBee1-Arduino is free software: you can redistribute it and/or modify
 * it under the terms of the GNU General Public License as published by
 * the Free Software Foundation, either version 3 of the License, or
 * (at your option) any later version.
 *
 * XBee1-Arduino is distributed in the hope that it will be useful,
 * but WITHOUT ANY WARRANTY; without even the implied warranty of
 * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
 * GNU General Public License for more details.
 *
 * You should have received a copy of the GNU General Public License
 * along with XBee1-Arduino.  If not, see <http://www.gnu.org/licenses/>.
 */
...
XBee11Response::XBee11Response() {

}

uint8_t XBee11Response::getApiId() {
	return _apiId;
}

void XBee11Response::setApiId(uint8_t apiId) {
	_apiId = apiId;
}

uint8_t XBee11Response::getMsbLength() {
	return _msbLength;
}

void XBee11Response::setMsbLength(uint8_t msbLength) {
	_msbLength = msbLength;
}

uint8_t XBee1Response::getLsbLength() {
	return _lsbLength;
}

void XBee1Response::setLsbLength(uint8_t lsbLength) {
	_lsbLength = lsbLength;
}

uint8_t XBee1Response::getChecksum() {
	return _checksum;
}

void XBee1Response::setChecksum(uint8_t checksum) {
	_checksum = checksum;
}

uint8_t XBee1Response::getFrameDataLength() {
	return _frameLength;
}

void XBee1Response::setFrameLength(uint8_t frameLength) {
	_frameLength = frameLength;
}

bool XBee1Response::isAvailable() {
	return _complete;
}

void XBee1Response::setAvailable(bool complete) {
	_complete = complete;
}

bool XBee1Response::isError() {
	return _errorCode > 0;
}

uint8_t XBee1Response::getErrorCode() {
	return _errorCode;
}

void XBee1Response::setErrorCode(uint8_t errorCode) {
	_errorCode = errorCode;
}

// copy common fields from XBee1 response to target response
void XBee1Response::setCommon(XBee1Response &target) {
...
void XBee1Response::getZBTxStatusResponse(XBee1Response &zbXBee1Response) {

	// way off?
	ZBTxStatusResponse* zb = static_cast<ZBTxStatusResponse*>(&zbXBee1Response);
	// pass pointer array to subclass
	zb->setFrameData(getFrameData());
	setCommon(zbXBee1Response);
}

ZBRxResponse::ZBRxResponse(): RxDataResponse() {
	_remoteAddress64 = XBee1Address64();
}
...
XBee1Address64& ZBRxResponse::getRemoteAddress64() {
	return _remoteAddress64;
}

void XBee1Response::getZBRxResponse(XBee1Response &rxResponse) {
...
void XBee1Response::getZBRxIoSampleResponse(XBee1Response &response) {
...
XBee1Address64& Rx64Response::getRemoteAddress64() {
	return _remoteAddress;
}

Rx64Response::Rx64Response() : RxResponse() {
	_remoteAddress = XBee1Address64();
}
...
//		int startIndex;
//
//		if (this.getSourceAddress() instanceof XBee1Address16) {
//			// 16 bit
//			startIndex = 7;
//		} else {
//			// 64 bit
...
void XBee1Response::getRx16IoSampleResponse(XBee1Response &response) {
...
XBee1Address64& Rx64IoSampleResponse::getRemoteAddress64() {
	return _remoteAddress;
}
...
void XBee1Response::getRx64IoSampleResponse(XBee1Response &response) {
...
void XBee1Response::getTxStatusResponse(XBee1Response &txResponse) {
...
void XBee1Response::getRx16Response(XBee1Response &rx16Response) {
...
void XBee1Response::getRx64Response(XBee1Response &rx64Response) {
...
XBee1Address64& RemoteAtCommandResponse::getRemoteAddress64() {
	return _remoteAddress64;
}

void XBee1Response::getRemoteAtCommandResponse(XBee1Response &response) {
...
RxDataResponse::RxDataResponse() : XBee1Response() {
...
void XBee1Response::getModemStatusResponse(XBee1Response &modemStatusResponse) {
...
void XBee1Response::getAtCommandResponse(XBee1Response &atCommandResponse) {
...
uint16_t XBee1Response::getPacketLength() {
	return ((_msbLength << 8) & 0xff) + (_lsbLength & 0xff);
}

uint8_t* XBee1Response::getFrameData() {
	return _frameDataPtr;
}

void XBee1Response::setFrameData(uint8_t* frameDataPtr) {
	_frameDataPtr = frameDataPtr;
}

void XBee1Response::init() {
	_complete = false;
	_errorCode = NO_ERROR;
	_checksum = 0;
}

void XBee1Response::reset() {
	init();
	_apiId = 0;
	_msbLength = 0;
	_lsbLength = 0;
...
void XBee1::resetResponse() {
	_pos = 0;
	_escape = false;
	_response.reset();
}

XBee1::XBee1(): _response(XBee1Response()) {
	_pos = 0;
	_escape = false;
	_checksumTotal = 0;
	_nextFrameId = 0;
...
void XBee1::begin(long baud) {
	_serial->begin(baud);
}

void XBee1::setSerial(SoftwareSerial &serial) {
	_serial = &serial;
}

bool XBee1::available() {
...
void XBee1::write(uint8_t val) {
	_serial->write(val);
}

XBee1Response& XBee1::getResponse() {
	return _response;
}

// TODO how to convert response to proper subclass?
void XBee1::getResponse(XBee1Response &response) {
...
void XBee1::readPacketUntilAvailable() {
...
bool XBee1::readPacket(int timeout) {
...
XBee1Request::XBee1Request(uint8_t apiId, uint8_t frameId) {
	_apiId = apiId;
	_frameId = frameId;
}

void XBee1Request::setFrameId(uint8_t frameId) {
	_frameId = frameId;
}

uint8_t XBee1Request::getFrameId() {
	return _frameId;
}

uint8_t XBee1Request::getApiId() {
	return _apiId;
}

void XBee1Request::setApiId(uint8_t apiId) {
	_apiId = apiId;
}

//void XBee1Request::reset() {
//	_frameId = DEFAULT_FRAME_ID;
//}

//uint8_t XBee1Request::getPayloadOffset() {
//	return _payloadOffset;
//}
//
//uint8_t XBee1Request::setPayloadOffset(uint8_t payloadOffset) {
//	_payloadOffset = payloadOffset;
//}


PayloadRequest::PayloadRequest(uint8_t apiId, uint8_t frameId, uint8_t *payload, uint8_t payloadLength) : XBee1Request(apiId, frameId) {
	_payloadPtr = payload;
...
XBee1Address::XBee1Address() {

}

XBee1Address64::XBee1Address64() : XBee1Address() {

}

XBee1Address64::XBee1Address64(uint32_t msb, uint32_t lsb) : XBee1Address() {
	_msb = msb;
	_lsb = lsb;
}

uint32_t XBee1Address64::getMsb() {
	return _msb;
}

void XBee1Address64::setMsb(uint32_t msb) {
	_msb = msb;
}

uint32_t XBee1Address64::getLsb() {
	return _lsb;
}

void XBee1Address64::setLsb(uint32_t lsb) {
	_lsb = lsb;
}
...
ZBTxRequest::ZBTxRequest(XBee1Address64 &addr64, uint16_t addr16, uint8_t broadcastRadius, uint8_t option, uint8_t *data, uint8_t dataLength, uint8_t frameId): PayloadRequest(ZB_TX_REQUEST, frameId, data, dataLength) {
	_addr64 = addr64;
	_addr16 = addr16;
...
ZBTxRequest::ZBTxRequest(XBee1Address64 &addr64, uint8_t *data, uint8_t dataLength): PayloadRequest(ZB_TX_REQUEST, DEFAULT_FRAME_ID, data, dataLength) {
	_addr64 = addr64;
...
XBee1Address64& ZBTxRequest::getAddress64() {
	return _addr64;
}
...
void ZBTxRequest::setAddress64(XBee1Address64& addr64) {
	_addr64 = addr64;
}
...
Tx64Request::Tx64Request(XBee1Address64 &addr64, uint8_t option, uint8_t *data, uint8_t dataLength, uint8_t frameId) : PayloadRequest(TX_64_REQUEST, frameId, data, dataLength) {
	_addr64 = addr64;
	_option = option;
}

Tx64Request::Tx64Request(XBee1Address64 &addr64, uint8_t *data, uint8_t dataLength) : PayloadRequest(TX_64_REQUEST, DEFAULT_FRAME_ID, data, dataLength) {
	_addr64 = addr64;
	_option = ACK_OPTION;
}
...
XBee1Address64& Tx64Request::getAddress64() {
	return _addr64;
}

void Tx64Request::setAddress64(XBee1Address64& addr64) {
	_addr64 = addr64;
}
...
AtCommandRequest::AtCommandRequest() : XBee1Request(AT_COMMAND_REQUEST, DEFAULT_FRAME_ID) {
	_command = NULL;
	clearCommandValue();
}

AtCommandRequest::AtCommandRequest(uint8_t *command, uint8_t *commandValue, uint8_t commandValueLength) : XBee1Request(AT_COMMAND_REQUEST, DEFAULT_FRAME_ID) {
	_command = command;
	_commandValue = commandValue;
	_commandValueLength = commandValueLength;
}

AtCommandRequest::AtCommandRequest(uint8_t *command) : XBee1Request(AT_COMMAND_REQUEST, DEFAULT_FRAME_ID) {
	_command = command;
	clearCommandValue();
}
...
XBee1Address64 RemoteAtCommandRequest::broadcastAddress64 = XBee1Address64(0x0, BROADCAST_ADDRESS);
...
RemoteAtCommandRequest::RemoteAtCommandRequest(XBee1Address64 &remoteAddress64, uint8_t *command, uint8_t *commandValue, uint8_t commandValueLength) : AtCommandRequest(command, commandValue, commandValueLength) {
...
RemoteAtCommandRequest::RemoteAtCommandRequest(XBee1Address64 &remoteAddress64, uint8_t *command) : AtCommandRequest(command, NULL, 0) {
	_remoteAddress64 = remoteAddress64;
	_remoteAddress16 = ZB_BROADCAST_ADDRESS;
	_applyChanges = false;
	setApiId(REMOTE_AT_REQUEST);
...
XBee1Address64& RemoteAtCommandRequest::getRemoteAddress64() {
	return _remoteAddress64;
}

void RemoteAtCommandRequest::setRemoteAddress64(XBee1Address64 &remoteAddress64) {
	_remoteAddress64 = remoteAddress64;
}
...
// TODO
//GenericRequest::GenericRequest(uint8_t* frame, uint8_t len, uint8_t apiId): XBee1Request(apiId, *(frame), len) {
//	_frame = frame;
//}

void XBee1::send(XBee1Request &request) {
	// the new new deal
...
void XBee1::sendByte(uint8_t b, bool escape) {
