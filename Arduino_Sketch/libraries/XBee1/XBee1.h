...
// set to ATAP value of XBee1. AP=2 is recommended
#define ATAP 2
...
/**
 * The super class of all XBee1 responses (RX packets)
...
private:
	void setCommon(XBee11Response &target);
	uint8_t _apiId;
	uint8_t _msbLength;
	uint8_t _lsbLength;
	uint8_t _checksum;
...
/**
 * Represents a 64-bit XBee11 Address
 */
class XBee11Address64 : public XBee11Address {
public:
	XBee11Address64(uint32_t msb, uint32_t lsb);
	XBee11Address64();
	uint32_t getMsb();
	uint32_t getLsb();
...
//class XBee11Address16 : public XBee11Address {
//public:
//	XBee11Address16(uint16_t addr);
//	XBee11Address16();
//	uint16_t getAddress();
 ...
 */
class FrameIdResponse : public XBee11Response {
public:
	FrameIdResponse();
	uint8_t getFrameId();
private:
 ...
 */
class RxDataResponse : public XBee11Response {
public:
	RxDataResponse();
	/**
...
public:
	ZBRxResponse();
	XBee1Address64& getRemoteAddress64();
...
private:
	XBee1Address64 _remoteAddress64;
};
 ...
 */
class ModemStatusResponse : public XBee1Response {
public:
	ModemStatusResponse();
	uint8_t getStatus();
};
...
/**
 * Super class of all XBee1 requests (TX packets)
...
public:
	 ...
	 * TODO make protected
	 */
	XBee1Request(uint8_t apiId, uint8_t frameId);
	/**
...
/**
 * Primary interface for communicating with an XBee1 Radio.
 * This class provides methods for sending and receiving packets with an XBee1 radio via the serial port.
 * The XBee1 radio must be configured in API (packet) mode (AP=2)
...
public:
	 ...
	 * You may call <i>XBee1</i>.getResponse().isAvailable() after calling this method to determine if
	 * a packet is ready, or <i>XBee1</i>.getResponse().isError() to determine if
	 * a error occurred.
	 * <p/>
	...
	void getResponse(XBee1Response &response);
	/**
	 * Returns a reference to the current response
	 * Note: once readPacket is called again this response will be overwritten!
	 */
	XBee1Response& getResponse();
	/**
	 * Sends a XBee1Request (TX packet) out the serial port
	 */
	void send(XBee1Request &request);
	//uint8_t sendAndWaitForResponse(XBee1Request &request, int timeout);
	/**
 ...
 */
class PayloadRequest : public XBee1Request {
public:
...
public:
	Tx64Request(XBee1Address64 &addr64, uint8_t option, uint8_t *payload, uint8_t payloadLength, uint8_t frameId);
	/**
	 * Creates a unicast Tx64Request with the ACK option and DEFAULT_FRAME_ID
	 */
	Tx64Request(XBee1Address64 &addr64, uint8_t *payload, uint8_t payloadLength);
	/**
	 * Creates a default instance of this class.  At a minimum you must specify
	 * a payload, payload length and a destination address before sending this request.
	 */
	Tx64Request();
	XBee1Address64& getAddress64();
	void setAddress64(XBee1Address64& addr64);
...
public:
	 ...
	 */
	ZBTxRequest(XBee1Address64 &addr64, uint8_t *payload, uint8_t payloadLength);
	ZBTxRequest(XBee1Address64 &addr64, uint16_t addr16, uint8_t broadcastRadius, uint8_t option, uint8_t *payload, uint8_t payloadLength, uint8_t frameId);
	/**
	 * Creates a default instance of this class.  At a minimum you must specify
	 * a payload, payload length and a destination address before sending this request.
	 */
 ...
 * The command is used to configure the serially connected XBee1 radio
 */
class AtCommandRequest : public XBee1Request {
public:
	AtCommandRequest();
 ...
 * The command is used to configure a remote XBee1 radio
 */
...
public:
	 ...
	 */
	RemoteAtCommandRequest(XBee1Address64 &remoteAddress64, uint8_t *command, uint8_t *commandValue, uint8_t commandValueLength);
	/**
	 * Creates a RemoteAtCommandRequest with 16-bit address to query a command.
	 * 16-bit address defaults to broadcast and applyChanges is true.
	 */
	RemoteAtCommandRequest(XBee1Address64 &remoteAddress64, uint8_t *command);
	uint16_t getRemoteAddress16();
	void setRemoteAddress16(uint16_t remoteAddress16);
	XBee1Address64& getRemoteAddress64();
	void setRemoteAddress64(XBee1Address64 &remoteAddress64);
	bool getApplyChanges();
	void setApplyChanges(bool applyChanges);
	uint8_t getFrameData(uint8_t pos);
	uint8_t getFrameDataLength();
	static XBee1Address64 broadcastAddress64;
